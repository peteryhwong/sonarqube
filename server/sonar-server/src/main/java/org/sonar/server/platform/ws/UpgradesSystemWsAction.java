...
public class UpgradesSystemWsAction implements SystemWsAction {
  ...
  public void handle(Request request, Response response) throws Exception {
    ...
    writeSonarUpgrades(jsonWriter, updateCenter);
  ...
  private void writeSonarUpgrades(JsonWriter jsonWriter, UpdateCenter updateCenter) {
    jsonWriter.name(ARRAY_UPGRADES).beginArray();

    for (SonarUpdate sonarUpdate : updateCenter.findSonarUpdates()) {
      writeSonarUpgrade(jsonWriter, sonarUpdate);
    }

    jsonWriter.endArray();
  }

  private void writeSonarUpgrade(JsonWriter jsonWriter, SonarUpdate sonarUpdate) {
    jsonWriter.beginObject();
